...
public abstract class AbstractPopulation<T extends IAgent> implements IPopulation<T> {
	...
	public abstract IList<T> createAgents(IScope scope, int number, List<? extends Map<String, Object>> initialValues,
			boolean isRestored, boolean toBeScheduled, IStatement sequence) throws GamaRuntimeException;
	...
	public IVariable[] orderAttributes(final ITypeDescription ecd, final Predicate<IVariableDescription> keep,
		...
		for (int i = 0; i < subs.size() - 1; i++) {
			IVariableDescription vs = subs.get(i);
			if (keep.apply(vs)) {
				IVariableDescription vt = subs.get(i + 1);
				if (keep.apply(vt)) {
